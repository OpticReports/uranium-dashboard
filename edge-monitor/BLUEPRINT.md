...
output is literally `insufficient: n=63 of 231`); BOCD posterior on
...
| YELLOW→GREEN | 20 trading days with all detectors below ½-threshold AND no new breach | restore size_mult 1.0 |
| YELLOW→RED | any: DD ≥p99; PSR(0)<0.05 post-MinTRL; 2nd independent detector within 30d; slip CUSUM re-alarm after reset | size_mult 0 (halt via engine's own halt path); quarantine ≥4 weeks; written review required |
...
once past MinTRL (PSR 0.5 → 0, PSR ≥0.9 → ~0.8+, capped by state table);
before MinTRL, size_mult follows the venue ramp (earn size), never full.
...
**False-alarm budget** (back-solved): RED <1 false per strategy / 3y →
DD p99 + Bonferroni on RED triggers; YELLOW ~2 false / strategy / yr →
ARL 500d CUSUM + p95 DD ≈ 1.7/yr expected under null (verified in the
calibration MC at registration; achieved rates logged to `checks`).
...
Every check returns one of `{verdict, insufficient(n, n_needed), undefined}`.
...
it logs only); slippage needs 10 trades; monthly-cadence books NEVER get
return-based verdicts (MinTRL in decades — printed once at registration so
nobody asks again); PBO `undefined:penalty_n=20` when the trial history
wasn't kept. "Insufficient" rows appear in the digest — silence is
...
| mo 3 | ibkr adapter (when executor ships); L3 quarterly template; RED tier armed after false-alarm audit | S5 PSR verdict ETA printed (~230 trading days from inception at SR≈1.2 — i.e., mid-2027; until then the system says so) |
