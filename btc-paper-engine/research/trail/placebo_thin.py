...
binding at 55 bars, so 13 of its 21 cells are byte-identical and the sweep
...
# thinning can only remove trades, so the matched range runs from the live
# cell's own count down to the grid's smallest cell
KEEPS = np.linspace(min(1.0, hi / N4) if hi < N4 else 1.0, lo / N4, len(GRID))
