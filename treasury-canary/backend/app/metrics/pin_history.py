...
def _parts_for_channel(cid: str, bundle: dict) -> list[tuple[str, tuple[list[date], list[float]]]]:
    if cid == "oil_shock":
        ...
        return [("Net oil price increase, 12m (vs 3-yr high)", nopi12_series(d, v)),
...
HISTORY_NOTES: dict[str, str] = {
    "oil_shock": "The NOPI trigger leg needs 48 months of daily WTI to warm up, so "
                 "on FRED's 1986+ series it starts 1989-12; the 12-month gauge covers "
...
def build_pin_history(bundle: dict) -> dict:
    ...
    return {
        ...
        "measured_roles": (
            ...
            "studies/pin-rule-hindcast v3, re-measured 2026-09-10): recession "
            "onsets belong to the yield curve — 30% precision, 4/4 onsets "
            "caught — and nothing built from pin reds reliably improved on it; "
            "raw convergence counts scored below base rate (10% vs 12%). Pin "
            "reds earn their keep on market accidents, more modestly than v2 "
            "claimed: fast-channel red on a flat/inverted curve preceded a "
            "≥15% drawdown start within 12m in 34% of months vs a 20% base "
            ...
            "drawdowns (hindcast v4, on the 12-month rule's windows before the "
            "2026-09-16 NOPI leg switch). On the full 1953–2025 record oil alone reads "
