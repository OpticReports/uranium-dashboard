...
export const GLOSSARY: Record<string, GlossaryEntry> = {
  ...
  leverage_corroboration: {
    ...
    calc: "Six flags, each computed live from current data: flat yield curve (10y−3mo < 1.0pp) · tightening cycle (3-month rate up >0.5pp over 12 months) · late expansion (≥48 months since the last recession) · low unemployment (<5%) · extended market (S&P up >50% over 3 years) · high excess (margin excess ≥ +25pp). Each chip renders its live reading against its bar; hover for the full reading and verdict. Flags with missing data are excluded from the denominator, not counted false.",
    read: "A dark 'tightening cycle' chip does NOT mean the Fed has not hiked — it tests the 12-month CHANGE in the 3-month rate, so a hike sitting in the same window as earlier cuts leaves it dark. The historical split: every blowoff with ≥4 flags lit (1967, 1998, 2000, 2007) was followed by a major bear — 4 of 4, est. 65–85% forward odds given the tiny sample. Blowoffs with ≤2 flags — the early-cycle re-leveraging kind, coming off a fresh recession with a steep curve — fizzled two-thirds of the time (4 of 12 became bears, ~33%). Unconditional base rate: 8 of 16 (~50%).",
    caveat: "Only ~16 blowoff episodes in 75 years, so these are small-sample estimates, not calibrated probabilities — treat the flag count as a lean, not a forecast. Thresholds were picked with the history in view, so the definitions are frozen — they are not re-tuned against a live reading. The 2026-09-21 audit re-derived the episode list and the 4-of-4 / 4-of-12 split from the shipped data, and re-scored every peak with the fed funds rate in place of the 3-month bill: identical. Any proposed change gets re-scored the same way before it ships (MARGIN_DEBT.md, Flag definition audit). The flags are descriptive of past cycles; a genuinely new regime (fiscal-dominance inflation, AI capex boom) can break the pattern in either direction.",
  },
  ...
  severity_index: {
    ...
    calc: "Six blocks, every component scored as a percentile of its full history (higher = more severe conditions). The ICE BofA series FRED truncated to three years in April 2026 are repaired with a frozen reference, so every rank here means what it says. Composite = 0.35·LeverageExcess + 0.25·WealthAtRisk + 0.20·Amplification + 0.20·BoomConcentration, adjusted +0.15·(PolicySpace−50) and −0.15·(Dampeners−50). Weights are literature-informed and fixed — with ~5 well-documented severe recessions, fitting weights would be fiction.",
  ...
  accident_gauge: {
    ...
    read: "GREEN = disarmed. YELLOW = armed (one condition met). RED = both — the configuration that, in the 1981–2026 hindcast (re-measured 2026-09-10), preceded the start of a ≥15% S&P drawdown within 12 months in 34% of months versus a 20% base rate; 4 of its 13 historical signal-clusters were followed by one, flagging LTCM 1998 four months early, 2019 eleven to seven months early, and 2025 twelve. It missed 2007: the earlier 44% / 5-of-11 figure counted a 2007 catch that came from a since-removed data artifact in the plumbing channel. The meter shows where the spread sits now (solid dot) and its 6-month low (hollow dot) relative to the red trip zone — watch the dots drift toward the line as the curve flattens.",
    caveat: "A high-conviction tripwire, not a net: it missed 2018 (curve stayed steep) and 2021 (policy-driven grind, the slow channels' domain). Both conditions and the +0.25pp threshold come from the study's pre-specified design (studies/pin-rule-hindcast v2) — nothing is fitted, ~11 clusters is a small sample, and 34% is descriptive history, not a calibrated probability. The sibling rule first reported as 'oil/policy window + flat curve' (45%, 5 of 6) turned out to be the oil channel's damage window alone (hindcast v4, 2026-09-15; the policy leg contributed one signal in its history): 48% on 4 of 5 clusters for recession onsets (3 legitimate) and 42% on 4 of 5 for drawdowns, measured on the 12-month rule's windows before the 2026-09-16 switch to the net-oil-price-increase leg — treat the pair as one family.",
  },
  pin_board: {
  ...
  pin_oil: {
    ...
    what: "Energy price spikes tax consumers and, historically, arrived together with Fed tightening. Re-measured on the full postwar record in September 2026 (studies/oil-shock-recession-weight.md, pre-registered and counter-agent verified): on its own oil is an accident tell and a flat-curve amplifier, not a recession forecaster.",
    calc: "Trigger leg: Hamilton's net oil price increase — each month's rise above the prior 3-year high (log-%), summed over the last 12 months of monthly-mean WTI. +10 = the study's pre-registered on-switch (new highs forming); +25 = its red anchor — 1990, 2000 and 2022 printed 52, 30 and 48, and 1990 was coincident with its onset, 2022 a false positive; 104 = the 1973–74 embargo peak. Context gauge, capped at yellow: the plain 12-month % change (+25 / +50).",
    read: "Measured 1953–2025: the trigger rule preceded 6 of 11 recession onsets with 3 named false positives (1976, 1996, 2021); the 12-month rule 3 of 11 with 7. Oil alone reads at the base rate on onsets (18% vs 17%), but a ≥15% S&P drawdown began within a year of a +50% print in 51% of months vs 23% base, and on a flat curve the oil damage window scored 48% on 4 of 5 clusters (1987+, 3 legitimate — the 2020 catch was the pandemic; measured on the 12-month rule's windows before the switch, the NOPI leg is re-scored after deploy).",
    caveat: "Oil adds NO out-of-sample skill to the curve probit (walk-forward 1970–2024, ΔAUC −0.015 to −0.035), so the recession dial never blends it — the study's pre-registered decision rule was not met. Hamilton's '10 of 11' counts 1948–2007 with rises of +7–10% and one coincident month. The US has been a net petroleum exporter since 2020; on 2009+ data a +50% shock's effect on onset odds is unidentified (a one-episode sample) and consistent with at most half the 1953–2008 effect, and 2022 fired on every definition with no recession. Mass is the ~$0.9T petroleum bill (~3% of GDP vs 8% in 1980), no longer $20T of consumption.",
  },
  pin_policy: {
  ...
  pin_private_credit: {
    ...
    calc: "Three reads. (1) CCC-and-lower bond spread, as a percentile of its own history since 1996 — the tier where refinancing distress prices first. FRED cut ICE BofA history to a rolling three years in April 2026, which silently turned this into a three-year rank; a frozen reference restores the real distribution, so it means since-1996 again. (2) The GAP between CCC and BBB spreads (percentile): investment grade priced for perfection while the distress tier cracks is a bifurcation the aggregate high-yield spread hides. (3) Bank lending to nondepository financial institutions (the Fed's H.8 breakout, added precisely to watch this) — month-over-month annualized growth.",
  ...
  pin_attributes: {
    ...
    calc: "Static, sourced numbers — documented exposure sizes and episode counts from the literature (e.g. the oil channel's measured 6-of-11 record, the CFTC-measured basis book). Deliberately NOT fitted weights: with ~11 postwar recessions and nine channels, any statistically fitted 'probability contribution' would be curve-fitting noise.",
