...
export const GLOSSARY: Record<string, GlossaryEntry> = {
  ...
  leverage_corroboration: {
    ...
    calc: "Six flags, each computed live from current data: flat yield curve (10y−3mo < 1.0pp) · tightening cycle (3-month rate up >0.5pp over 12 months) · late expansion (≥48 months since the last recession) · low unemployment (<5%) · extended market (S&P up >50% over 3 years) · high excess (margin excess ≥ +25pp). Hover any chip for its live reading and the bar it is judged against. Flags with missing data are excluded from the denominator, not counted false.",
    read: "A dark 'tightening cycle' chip does NOT mean the Fed has not hiked — it tests the 12-month CHANGE in the 3-month rate, so a hike sitting in the same window as earlier cuts leaves it dark (Sep 2026: +0.10pp against a +0.50pp bar, one hike against two cuts). The historical split: every blowoff with ≥4 flags lit (1967, 1998, 2000, 2007) was followed by a major bear — 4 of 4, est. 65–85% forward odds given the tiny sample. Blowoffs with ≤2 flags — the early-cycle re-leveraging kind, coming off a fresh recession with a steep curve — fizzled two-thirds of the time (4 of 12 became bears, ~33%). Unconditional base rate: 8 of 16 (~50%).",
    caveat: "Only ~16 blowoff episodes in 75 years, so these are small-sample estimates, not calibrated probabilities — treat the flag count as a lean, not a forecast. Thresholds were picked with the history in view, and the per-episode scoring behind the 4-of-4 statistic is not reproducible from this repo's series, so the definitions are frozen: they are not re-tuned against a live reading (audit: MARGIN_DEBT.md, 2026-09-21). The flags are descriptive of past cycles; a genuinely new regime (fiscal-dominance inflation, AI capex boom) can break the pattern in either direction.",
  },
