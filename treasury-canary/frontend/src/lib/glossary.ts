...
export const GLOSSARY: Record<string, GlossaryEntry> = {
  ...
  severity_index: {
    ...
    calc: "Six blocks, every component scored as a percentile of its full history (higher = more severe conditions). One component, HY spread complacency, ranks against a frozen 1996-2026 reference distribution rather than the live feed, because FRED cut ICE BofA history to a rolling 3 years in April 2026 — without that the score would silently mean \"tightest in 3 years\". Composite = 0.35·LeverageExcess + 0.25·WealthAtRisk + 0.20·Amplification + 0.20·BoomConcentration, adjusted +0.15·(PolicySpace−50) and −0.15·(Dampeners−50). Weights are literature-informed and fixed — with ~5 well-documented severe recessions, fitting weights would be fiction.",
  ...
  pin_private_credit: {
    ...
    calc: "Four reads. (1) The CCC-and-lower bond spread LEVEL — the tier where refinancing distress prices first, and the only leg that can take this channel RED. Anchored on documented levels: 4.14% is the series record low (Jun-2007), 44.29% the record high (Dec-2008), and 10% is Fridson's distress convention (at or above +1000bps). (2) That same spread as a percentile — but of a ROLLING ~3-YEAR window, not since 1996: FRED restricted ICE BofA history in April 2026, so it reads as worst-in-3-years and is capped at YELLOW. (3) The GAP between CCC and BBB spreads (percentile, same 3-year caveat, also capped at YELLOW): investment grade priced for perfection while the distress tier cracks is a bifurcation the aggregate high-yield spread hides. (4) Bank lending to nondepository financial institutions (the Fed's H.8 breakout, added precisely to watch this) — month-over-month annualized growth.",
    read: "Private-credit marks are opaque and lag by quarters; public CCC bonds and the funding pipe are the real-time windows. Spread LEVEL at or above the distress convention + dispersion extreme + NDFI loan growth stalling together = the private-credit margin call arriving. Any one alone is context.",
